...
static void game_meeting_events(GameContext *ctx)
{
    ...
    handle_send_vote_button(ctx->client, ctx->renderer, &ctx->event, ctx->state->players[ctx->local_id].isAlive, ctx->targeted_banner_id, ctx->state->players[ctx->local_id].player_voted, ctx->local_id);
}
